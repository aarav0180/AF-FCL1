"""
Generator: CIFAR100 Setting 2 (D.2 challenging setting).
Paper config: N=8 clients, T=6 tasks, C=10 classes/task, 400 samples/class.
Each client gets independent random classes (60 classes per client out of 100).
Output: datasets/PreciseFCL/data_split/CIFAR100_cn8_tn6_cet10_s2571.pkl
"""
...
CLASSES_PER_CLIENT = N_TASKS * CLASSES_PER_TASK  # 60
SEED = 2571

DATA_DIR = "datasets/PreciseFCL/"
SPLIT_DIR = os.path.join(DATA_DIR, "data_split")
SAVE_PATH = os.path.join(SPLIT_DIR, "CIFAR100_cn8_tn6_cet10_s2571.pkl")
...
for client_id in range(N_CLIENTS):
    # Each client randomly samples 60 classes out of 100
    client_classes = np.random.choice(100, size=CLASSES_PER_CLIENT, replace=False)
    c_train, c_test, c_labels = [], [], []
    for t in range(N_TASKS):
        task_classes = client_classes[t * CLASSES_PER_TASK:(t + 1) * CLASSES_PER_TASK]
    ...
    print(f"  Client {client_id}: {CLASSES_PER_CLIENT} classes, {N_TASKS} tasks")
