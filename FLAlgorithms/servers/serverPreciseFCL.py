...
#                     assert (self.users[i].id == id)
        ...
        if self.research_tracker is not None:
            ...
            # Classifier bias metrics (B_cls, B_norm)
            self.research_tracker.compute_classifier_bias(self.model)
        ...
        for user in self.selected_users:
            for name, param in user.model.named_parameters():
                if ('fc_classifier' in name and class_partial):
                    class_available = torch.Tensor(user.classes_so_far).long().to(self.device)
                    param_dict[name][class_available] += param.data[class_available] * user.train_samples / total_train
                    
                    add_weight = torch.zeros([param.data.shape[0]]).to(self.device)
