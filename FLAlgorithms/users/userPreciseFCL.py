...
class UserPreciseFCL(User):
    ...
    def next_task(self, train, test, label_info = None, if_label = True):
        ...
        # Compute EWC Fisher before deepcopy (if model supports it)
        if hasattr(self.model, 'compute_ewc_fisher'):
            device = self._device()
            self.model.compute_ewc_fisher(self.trainloaderfull, device)
    ...
    def train(
        ...
        # device - Respect args.device parameter
        if self.args.device == 'cuda':
            if torch.cuda.is_available():
                device = torch.device('cuda:0')
            else:
    ...
    def test_all_(self, personal=False, matrix=False):
        ...
        # CRITICAL: torch.no_grad() prevents gradient graph creation during eval,
        # which was the main cause of CUDA Out of Memory errors.
        with torch.no_grad():
            for test_loader in self.test_data_so_far_loader:
                ...
                for x, y in test_loader:
                    ...
                    if matrix == True:
                        # confusion matrix — move to CPU to avoid GPU accumulation
