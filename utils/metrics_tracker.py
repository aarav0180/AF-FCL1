...
class ResearchMetricsTracker:
    ...
    def __init__(self, output_dir: str, num_tasks: int, num_clients: int,
        ...
        # Per-client per-task: a_{t,i}_k and n_k^i for weighted formulas
        self.client_task_acc_history = []   # list[t] of ndarray[N_clients, tasks_seen]
        self.client_task_samples_history = []  # list[t] of ndarray[N_clients, tasks_seen]
    ...
    def record_task_accuracy(self, task_idx: int, task_accs: np.ndarray,
                             ...
                             user_task_num_samples: np.ndarray = None,
                             glob_acc: float = None):
        """
        ...
        user_task_num_samples : np.ndarray, shape [num_users, num_tasks_seen_so_far]
            Per-user per-task sample counts (optional, needed for Eq.10/11).
        glob_acc : float
        ...
        if user_task_acc is not None:
            ...
            self.client_task_acc_history.append(user_task_acc.copy())

        if user_task_num_samples is not None:
            self.client_task_samples_history.append(user_task_num_samples.copy())
    ...
    def compute_weighted_average_accuracy(self) -> float:
        """
        Paper Eq. 10: Weighted Average Accuracy.
        AA = (1 / sum n_k^i) * sum_k sum_i a_{T,i}_k * n_k^i
        """
        T = self.num_tasks
        if len(self.client_task_acc_history) < T or len(self.client_task_samples_history) < T:
            # Fallback to simple average
            final_acc = self.accuracy_matrix[T - 1, :]
            return float(np.nanmean(final_acc))

        # a_{T,i}_k: per-client per-task accuracy after learning all tasks
        final_acc = self.client_task_acc_history[T - 1]    # [N_clients, T]
        final_samples = self.client_task_samples_history[T - 1]  # [N_clients, T]

        total_samples = np.sum(final_samples)
        if total_samples == 0:
            return 0.0
        return float(np.sum(final_acc * final_samples) / total_samples)

    def compute_weighted_average_forgetting(self) -> float:
        """
        Paper Eq. 11: Weighted Average Forgetting.
        AF = (1 / sum_{k,i<T} n_k^i) * sum_k sum_{i=0}^{T-2} max_{t in [0..T-2]}(a_{t,i}_k - a_{T,i}_k) * n_k^i
        """
        T = self.num_tasks
        if T <= 1:
            return 0.0
        ...
        N = self.num_clients
        final_acc = self.client_task_acc_history[T - 1]        # [N, T]
        final_samples = self.client_task_samples_history[T - 1]  # [N, T]

        weighted_forgetting_sum = 0.0
        weight_sum = 0.0

        for i in range(T - 1):  # tasks 0..T-2
            for k in range(N):
                n_k_i = final_samples[k, i]
                # Find max a_{t,i}_k for t in [0..T-2]
                best_past = -np.inf
                for t in range(T - 1):  # t in [0..T-2]
                    if t < len(self.client_task_acc_history):
                        acc_arr = self.client_task_acc_history[t]
                        if k < acc_arr.shape[0] and i < acc_arr.shape[1]:
                            best_past = max(best_past, acc_arr[k, i])
                if best_past == -np.inf:
                    continue
                a_T_i_k = final_acc[k, i] if i < final_acc.shape[1] else 0.0
                weighted_forgetting_sum += max(0.0, best_past - a_T_i_k) * n_k_i
                weight_sum += n_k_i

        if weight_sum == 0:
            return 0.0
        return float(weighted_forgetting_sum / weight_sum)

    def compute_classifier_bias(self, model):
        """
        Compute classifier bias metrics:
          B_cls  = Var(||W_c||)           — classifier norm variance
          B_norm = Std(||W_c||) / Mean(||W_c||) — normalized classifier bias
        Where W_c is the weight vector for class c in the final classifier layer.
        """
        fc_weight = None
        classifier = model.classifier
        if hasattr(classifier, 'fc_classifier'):
            fc_weight = classifier.fc_classifier.weight.data.clone().cpu().numpy()
        else:
            # Search for the last Linear layer
            for name, module in classifier.named_modules():
                if isinstance(module, torch.nn.Linear):
                    fc_weight = module.weight.data.clone().cpu().numpy()

        if fc_weight is None:
            logger.warning("[ResearchMetrics] Could not find classifier weights for bias metrics")
            return

        # ||W_c|| for each class c
        weight_norms = np.linalg.norm(fc_weight, axis=1)  # shape [num_classes]

        b_cls = float(np.var(weight_norms))
        mean_norm = float(np.mean(weight_norms))
        b_norm = float(np.std(weight_norms) / mean_norm) if mean_norm > 0 else 0.0

        self.bias_results = {
            'B_cls': b_cls,
            'B_norm': b_norm,
            'weight_norms_mean': mean_norm,
            'weight_norms_std': float(np.std(weight_norms)),
        }

        logger.info("[ResearchMetrics] Classifier Bias: B_cls=%.6f  B_norm=%.6f",
                    b_cls, b_norm)

        if self.tb_writer is not None:
            self.tb_writer.add_scalar('bias/B_cls', b_cls, 0)
            self.tb_writer.add_scalar('bias/B_norm', b_norm, 0)
    ...
    def _save_summary_json(self):
        ...
        weighted_acc = self.compute_weighted_average_accuracy()
        weighted_fgt = self.compute_weighted_average_forgetting()
        ...
        summary = {
            ...
            'num_clients': self.num_clients,
            'accuracy': {
                'weighted_average_accuracy_eq10': weighted_acc,
    ...
    def print_final_summary(self):
        ...
        # Accuracy (paper Eq. 10)
        logger.info("\n--- Accuracy (Paper Eq. 10: Weighted) ---")
        logger.info("  Weighted Average Accuracy:   %.4f %%", weighted_acc * 100)
        logger.info("  Simple Average Accuracy:     %.4f %%", avg_acc * 100)
        for i in range(self.num_tasks):
            val = final_acc[i]
            logger.info("    Task %d:  %.4f %%", i,
                        val * 100 if not np.isnan(val) else 0.0)

        # Forgetting (paper Eq. 11)
        logger.info("\n--- Forgetting (Paper Eq. 11: Weighted) ---")
        logger.info("  Weighted Average Forgetting: %.4f", weighted_fgt)
        logger.info("  Simple Average Forgetting:   %.4f", np.mean(forgetting))
        for i, f_i in enumerate(forgetting):
            logger.info("    Task %d:  %.4f", i, f_i)

        # Bias Metrics
        logger.info("\n--- Classifier Bias Metrics ---")
        if self.bias_results:
            logger.info("  B_cls  (Var(||W_c||)):       %.6f", self.bias_results.get('B_cls', 0.0))
            logger.info("  B_norm (Std/Mean(||W_c||)):   %.6f", self.bias_results.get('B_norm', 0.0))
        else:
            logger.info("  (no bias data recorded)")
